        ...
        <div class="content">
            <div class="page-header">
                <div class="row">
                    <div class="col-sm-12">
                        <ul class="breadcrumb">
                            <li class="breadcrumb-item"><a href="{{ route('company.appointments') }}">Appointments</a></li>
            ...
            <div class="row">
                <div class="col-lg-12">
                    <div class="card-box">
                        ...
                        <form action="{{ route('company.appointments.update', $appointment->id) }}" method="POST">
                            @csrf
                            @method('PUT')
                            <div class="row">
                                    ...
                                    <select class="form-control" name="team_id" id="team_id" required>
                                        ...
                                        @foreach ($teams as $team)
                                            <option value="{{ $team->id }}"
                                                {{ $appointment->team_id == $team->id ? 'selected' : '' }}>
                                                {{ $team->title }}</option>
                                        @endforeach
                                    </select>
                                    @error('team_id')
        ...
        $(document).ready(function() {
            function fetchStaffByTeam(teamId) {
                if (teamId) {
                    $.ajax({
                        type: "GET",
                        url: "{{ route('company.appointments.getStaffByTeam') }}",
                        data: {
                            team_id: teamId
                        },
