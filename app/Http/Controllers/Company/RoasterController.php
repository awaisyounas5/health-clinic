...
class RoasterController extends Controller
{
    public function index()
    {
        ...
        return view('company.roaster.roaster', compact('teams', 'patients'));
    }
    ...
    public function events(Request $request)
    {
        try {
            ...
            $shift_types = ShiftType::where('team_id', $request->filter)->where('created_by', Auth::id())->get();
            $html = view('ajax.staff_member', compact('staffs'))->render();
            $team_shift = view('ajax.shift_type', compact('shift_types'))->render();

            $shift = Shift::where('team_id', $request->filter)->where('created_by', Auth::user()->id)->get();
    ...
    {
        try {
            $payrates = PayRate::where('staff_id', $request->staff_member)->where('created_by', Auth::id())->get();
    ...
    public function event_edit(RoasterRequest $request, $id)
    {
        ...
        try {
            ...
            DB::beginTransaction();
            $shift = Shift::where('id', $id)->where('created_by', Auth::id())->update([
            ...
            DB::commit();
            $shift = Shift::where('team_id', $request->team_id)->where('created_by', Auth::user()->id)->get();
            $events = $shift->map(function ($shift) {
                return [
                    'id'    => $shift->id,
                    'team_id' => $shift->team_id,
    ...
    {
        try {
            $shift = Shift::where('id', $id)->where('created_by', Auth::user()->id)->first();
            if ($shift) {
                $shift->delete();
            }
    ...
    public function publish_events($id)
    {
        try {
            ...
            Shift::where('team_id', $id)->where('created_by', Auth::user()->id)->update([
                'status' => 'A',
            ]);

            $shift = Shift::where('team_id', $id)->where('created_by', Auth::user()->id)->get();
            $events = $shift->map(function ($shift) {
                return [
                    'id'    => $shift->id,
                    'team_id' => $shift->team_id,
